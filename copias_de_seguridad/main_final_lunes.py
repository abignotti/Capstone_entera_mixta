...
for i in I_extra:
    ...
    C[i]  = M       # un umbral muy alto para que el extra se comporte como nuevo
...
# 3.7. Variable de swap: z[i,t]=1 si el motor i cambia de avión entre t-1 y t
z = model.addVars(I_WB, T, vtype=GRB.BINARY, name="z")


# ─── (4) RESTRICCIONES ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────

# 4.0.1 No puede haber swap en t=1
for i in I_WB:
    model.addConstr(z[i, 1] == 0, name=f"no_swap_init_{i}")


# 4.0.2 Asignación inicial en t=1
for i in I_WB:
    if i <= n_aviones:
...
# 4.8 Continuidad con swap: el motor solo puede cambiar de avión
# si entra en mantenimiento o hay un swap z[i,t]=1
for i in I_WB:
    for p in P_WB:
        for t in T[1:]:
            model.addConstr(
                ...
                >= a[i, p, t-1]
                  - m[i, t]
                  - z[i, t],
                name=f"continuity_with_swap_{i}_{p}_{t}"
            )
...
# 4.13 Frecuencia de swaps: máximo 1 swap en cualquier periodo de 2 semanas
#    (ventanas solapadas: semanas t y t+1)
for t in T:
    if t < max(T):
        model.addConstr(
            # sumamos z[i,t] + z[i,t+1] para todos los motores i
            quicksum(z[i, t] for i in I_WB)
          + quicksum(z[i, t+1] for i in I_WB)
            <= 1,
            name=f"swap_freq_{t}_{t+1}"
        )

# 4.14 Detectar swaps (versión ligera): si cambia la asignación, z=1
for i in I_WB:
    for t in T[1:]:
        for p in P_WB:
            # salida de p
            model.addConstr(z[i, t] >= a[i, p, t-1] - a[i, p, t],
                            name=f"swap_out_{i}_{p}_{t}")
            # llegada a p
            model.addConstr(z[i, t] >= a[i, p, t]   - a[i, p, t-1],
                            name=f"swap_in_{i}_{p}_{t}")
...
# ─── (6) OPTIMIZACIÓN Y EXPORTACIÓN DE RESULTADOS ────────────────────────────────────────────────────────────────────────

model.optimize()

if model.status == GRB.OPTIMAL:
    # ---- EXPORTAR plane_weekly_status.csv ----
    records_plane = []
    ...
    for t in T:
        for p in P_WB:
            ...
            records_plane.append({
                'Semana': t,
                'Avion_ID': p,
                'Motor_asignado':    id2mat.get(motor),
    ...
    pd.DataFrame(records_plane).to_csv('processed_data/plane_weekly_status.csv', index=False)

    # ---- EXPORTAR weekly_report.csv ----
    records_weekly = []
    cum_cost = 0.0
    for t in T:
        # acumular coste
        ...
        n_swap  = 0
        n_over  = sum(1 for rec in records_plane if rec['Semana']==t and rec['OverThreshold'])
    ...
    pd.DataFrame(records_weekly).to_csv('processed_data/weekly_report.csv', index=False)

    print("Archivos CSV generados correctamente.")

elif model.status == GRB.INFEASIBLE:
    print("¡El modelo es infactible! Generando IIS…")
    model.computeIIS()
    model.write("model.ilp")
    sys.exit(1)

else:
    print(f"Solver terminó con estado {model.status}")
    sys.exit(2)
